...
- Palette figée, déjà en variables CSS. Site public (`--r-*`, `assets/css/base.css`) : crème (`--r-cream` `#f8f4ec`), ardoise chaude (`--r-dark` `#343b3d`), or (`--r-gold` `#c9a86a`), sauge (`--r-sage` `#5d7358`), os et encres (`--r-bone`, `--r-ink`), filets (`--r-line`). Administration (`--ad-*`, `assets/css/admin.css`) : mint doux et blanc, or réservé à l'action principale et à l'état publié, teal (Teal Sanctuaire) pour la navigation et le focus, vert forêt d'origine réduit à l'encre des titres. Un seul thème, pas de bascule sombre. Aucune couleur hors palette sans variable déclarée.
