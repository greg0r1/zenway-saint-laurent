...
- Palette figée, déjà en variables CSS. Site public (`--r-*`, `assets/css/base.css`) : crème (`--r-cream` `#f8f4ec`), ardoise chaude (`--r-dark` `#343b3d`), or (`--r-gold` `#c9a86a`), sauge (`--r-sage` `#5d7358`), os et encres (`--r-bone`, `--r-ink`), filets (`--r-line`). Administration (`--ad-*`, `assets/css/admin.css`) : identité propre, d'après une maquette fournie et validée par l'utilisateur — gris-bleu (`#f5f8f9`), blanc, et **Sarcelle Ardoise** (`#427482`) pour tout ce qui agit ou situe. L'or et le Cormorant du site public n'y entrent pas ; les titres de l'admin sont en DM Sans gras capitales. Un seul thème, pas de bascule sombre. Aucune couleur hors palette sans variable déclarée.
- Deux polices, auto-hébergées, aucune autre : Cormorant Garamond (titres **du site public**), DM Sans (texte courant, interface, et **toute** l'administration, titres compris). Une troisième, Caveat, a été retirée : déclarée de longue date, elle n'habillait aucun texte.
